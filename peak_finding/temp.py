...
def saveCheetahStaticMask(runDir, detInfo, combinedMask, tag=None):
    # Save cheetah format mask
    if 'cspad' in detInfo.lower():# and 'cxi' in self.parent.experimentName:
        dim0 = 8 * 185
        dim1 = 4 * 388
    elif 'rayonix' in detInfo.lower():# and 'mfx' in self.parent.experimentName:
        dim0 = 1920
        dim1 = 1920
    else:
        print "#####################################"
    ...
    if tag is not None:
        tag = "_"+tag
    fname = runDir + "/staticMask"+tag+".h5"
    ...
    if combinedMask is None:
        ...
        counter = 0
        if 'cspad' in detInfo.lower():# and 'cxi' in self.parent.experimentName:
            img = utils.pct(combinedMask)
        elif 'rayonix' in detInfo.lower():# and 'mfx' in self.parent.experimentName:
            img = combinedMask[counter, :, :]  # psana format
    dset[:, :] = img
    myHdf5.close()
...
if rank == 0:
    ...
    import psana
    experimentName = 'mfxc00318'
    runNumber = '10'
    detInfo = 'MfxEndstation.0:Epix10ka2M.0'
    ds = psana.DataSource('exp='+experimentName+':run='+runNumber+':idx')
    run = ds.runs().next()
    det = psana.Detector(detInfo)
    times = run.times()
    env = ds.env()
    psanaMask = det.mask(par, calib=args.psanaMask_calib, status=args.psanaMask_status, edges=args.psanaMask_edges, central=args.psanaMask_central, unbond=args.psanaMask_unbon, unbondnbrs=args.psanaMask_unbondnrs, unbondnbrs8=False)

    saveCheetahStaticMask(args.outDir, args.det, psanaMask, tag=args.tag)
