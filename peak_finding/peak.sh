...
#export PATH=/project/projectdirs/chuck/Software/psocake/app:$PATH
#export PYTHONPATH=/project/projectdirs/chuck/Software/psocake:$PYTHONPATH
...
runNum=$(printf "%04d" $RUN)
mkdir -p $outdir/r$runNum

python -u findPeaksCori1time.py -e cxic0515 -d CxiDs2.0:Cspad.0 --outDir $outdir/r$runNum --algorithm 2 --alg_npix_min 2.0 --alg_npix_max 30.0 --alg_amax_thr 300.0 --alg_atot_thr 600.0 --alg_son_min 10.0 --alg1_thr_low 0.0 --alg1_thr_high 0.0 --alg1_rank 3 --alg1_radius 3 --alg1_dr 2 --psanaMask_on True --psanaMask_calib True --psanaMask_status True --psanaMask_edges True --psanaMask_central True --psanaMask_unbond True --psanaMask_unbondnrs True --mask $outdir/r$runNum/staticMask.h5 --clen CXI:DS2:MMS:06.RBV --coffset 0.3189938 --minPeaks 15 --maxPeaks 2048 --minRes -1 --sample sample --instrument CXI --pixelSize 0.00010992 --auto False --detectorDistance 0.144 --access ana -r $RUN --noe 80 
