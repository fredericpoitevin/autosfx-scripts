...
def saveCheetahFormatMask(run, arg):
    if arg == 'LCLS':
        if 'cspad' in args.det.lower():# and 'cxi' in self.parent.experimentName:
            dim0 = 8 * 185
            dim1 = 4 * 388
        elif 'rayonix' in args.det.lower():# and 'mfx' in self.parent.experimentName:
            dim0 = 1920
            dim1 = 1920
        elif 'epix10k' in args.det.lower() and '2m' in args.det.lower():
            dim0 = 16 * 352
            dim1 = 1 * 384
        elif 'jungfrau4m' in args.det.lower():
            dim0 = 8 * 512
            dim1 = 1 * 1024
        else:
        ...
        ds = psana.DataSource("exp="+args.exp+":run="+args.run+":idx")
        runs = ds.runs().next()
        times = runs.times()
        env = ds.env()
        if 'jungfrau4M' in args.det:
            psanaMask = det.mask(runs, calib=True, status=True, edges=False, central=False, unbond=False, unbondnbrs=False)
        else:
            psanaMask = det.mask(runs, calib=True, status=True, edges=True, central=True, unbond=True, unbondnbrs=False)
        combinedMask = psanaMask

        fname = args.outDir+'/r'+str(run).zfill(4)+'/staticMask.h5'
        ...
        if combinedMask is None:
            ...
            counter = 0
            if 'cspad' in args.det.lower():# and 'cxi' in self.parent.experimentName:
                img = pct(combinedMask)
            elif 'rayonix' in args.det.lower():# and 'mfx' in self.parent.experimentName:
                img = combinedMask[:, :] # psana format
            elif 'epix10k' in args.det.lower() and '2m' in args.det.lower():
                img = pct_epix10k(combinedMask)
            elif 'jungfrau4m' in args.det.lower():
                img = pct_jungfrau(combinedMask)
        dset[:,:] = img
        myHdf5.close()
