...
parser.add_argument("--splitIndex", help="index in the list of CXI files", default="0", type=str)
# PAL specific
...
def getIndexedPeaks():
    totalStream = runDir + "/" + experimentName + "_" + str(runNumber).zfill(4)
    if args.splitIndex:
        totalStream += "_" + args.splitIndex
    if tag:
        totalStream += "_" + tag
...
if args.splitIndex: peakFile += '_'+args.splitIndex
if args.pkTag: peakFile += '_'+args.pkTag
peakFile += '.cxi'
fnameIndex = runDir+"/status_index"
if args.splitIndex: fnameIndex += '_'+args.splitIndex
...
if hasData:
    ...
    if facility == 'LCLS':
        ...
        for rank in range(numWorkers):
            ...
            cmd = 'srun -n 1 --mem=5000 --gres=craynetwork:0 --cpus-per-task='+str(coreNumber)+' -o '+_log+' shifter /global/project/projectdirs/lcls/SFX_automation/entrypoint.sh '
